...
SegmentTree<ll, ll> RangeSumTree(int n) {
    auto fm = [](ll a, ll b) { return (a + b) % mod; };
    auto fl = [](ll a, ll b) { return (a + b) % mod; };
    auto fa = [](ll a, ll b) { return (a + b) % mod; };
    auto fw = [](ll a, ll w) { return (a * w) % mod; };
...
void solve() {
    ll n, k;
    cin >> n >> k;
    SegmentTree<ll, ll> tree = RangeSumTree(n + 1);
    vl imos(n + 2);
    rep(i, k) {
        ll l, r;
        cin >> l >> r;
    ...
    vector<P> lrs;
    rep(i, n + 2) {
        if (cnt == 0 && imos[i] > 0) start = i;
        ll newCnt = cnt + imos[i];
        if (cnt > 0 && newCnt == 0) lrs.emplace_back(P{start, i - 1});
        cnt = newCnt;
    }
    ...
    rep(i, n) {
        each(e, lrs) {
            ll l = i + e.first, r = i + e.second;
            if (l > n - 1) continue;
            r = min(n - 1, r);
            tree.update(l, r + 1, tree.query(i, i + 1));
        }
    }

    cout << tree.query(n - 1, n) << '\n';
}
