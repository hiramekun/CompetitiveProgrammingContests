/**
 * Created by hiramekun at 21:50 on 2020-05-20.
 */
#include <bits/stdc++.h>
...
void solve() {
    ...
    ll q;
    cin >> q;
    SegmentTree<ll, ll> tree(n + 1, [](ll a, ll b) { return (a | b); }, 0LL);
    rep(i, n) {
        tree.update(i, 1LL << (s[i] - 'a'));
    }
    while (q--) {
        ll code;
        cin >> code;
        switch (code) {
            case 1:
                ...
                cin >> i >> c;
                i--;
                tree.update(i, 1LL << (c - 'a'));
                break;
            case 2:
                ll l, r;
                cin >> l >> r;
                l--, r--;
                ll cnt = tree.query(l, r + 1);
                ll ans = 0;
                while (cnt) {
                    if (cnt & 1) ans++;
                    cnt = (cnt >> 1);
