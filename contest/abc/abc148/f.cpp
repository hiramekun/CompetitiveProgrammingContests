/**
 * Created by hiramekun at 21:40 on 2019-12-22.
 */
#include <bits/stdc++.h>
...
public:
    ...
    // O(v + e)
    // 閉路が存在するときは-1を返す
    ll get_longest_path(ll s, ll penalty) {
        vector<bool> seen(v);
        vl d(v, 0);
...
void solve() {
    ...
    vl t_table = gt.dijkstra(u);
    vl a_table = ga.dijkstra(v);

    ll top = 0;
    ll diff = ll_inf;
    rep(i, n) {
        if (a_table[i] == 0 || t_table[i] == 0) continue;
        ll tmp = a_table[i] - t_table[i];
        if (tmp >= 0 && tmp < diff) {
            diff = tmp;
            top = i;
        }
    }
    vl tpath = gt.get_path(top), apath = ga.get_path(top);
    ll ttop = top, atop = top;
    if (diff % 2 != 0) {
        tpath.pop_back();
        ttop = tpath.back();
    } else {
    ...
    ll ans = gt.get_longest_path(ttop, atop);
    ans += t_table[top];
    cout << ans << '\n';
}
