/**
 * Created by hiramekun at 21:24 on 2020-04-02.
 */
#include <bits/stdc++.h>
...
void solve() {
    ...
    vector<char> s(n + 1);
    cin >> s;
    SegmentTree<ll, ll> tree(n + 2, [](ll a, ll b) { return min(a, b); }, ll_inf);
    tree.update(0, 0);
    rep(i, n + 1) {
        if (s[i] == '0') {
            ll val = tree.query(max(0LL, i - m), i + 1);
            tree.update(i, val + 1);
        } else {
        }
    }
    cout << tree.query(n, n + 1) << '\n';
}
